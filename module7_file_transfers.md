...
- 💡 Use `python3 -m http.server 8080` for quick one‑liner serving current directory.
- 🔄 Prefer `scp -C` to enable compression for large transfers over slow links.
- 🛠️ Test upload endpoints with `curl -F` to spot misconfigured file‑type filters.
